...
from .utils import if_2d_array, if_model_selection, if_all_none, list_to_array, if_non_conforming
...
def solve_non_conforming(S: np.ndarray, N: list, G: np.ndarray, latent: bool = None, model_selection: bool = None,
                         reg: str = 'GGL', lambda1: list = None, lambda2: list = None, mu1: list = None):

    if model_selection:
        print("\tDD MODEL SELECTION:")
        modelselect_params = {'lambda1_range': lambda1, 'lambda2_range': lambda2, 'mu1_range': mu1}
        P = glasso_problem(S, N=N, G=G, latent=latent, reg=reg)
