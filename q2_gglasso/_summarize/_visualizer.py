...
def _make_heatmap(
    ...
    bokeh_tools = [
        "save, zoom_in, zoom_out, wheel_zoom, box_zoom, crosshair, reset, hover"
    ]
...
def _make_stats(solution: zarr.hierarchy.Group, labels_dict: dict = None):
    ...
    columns_best = [
        TableColumn(field=col_ix, title=col_ix) for col_ix in df_best.columns
    ]
...
def update_labels_dict(labels_dict, clust_order, n_cov=None):
    ...
    if n_cov is None:
        ...
        labels_dict_reversed = {
            len(labels_asvs) - 1 - k: v for k, v in labels_dict.items()
        }
    else:
        labels_asvs = {i: labels_dict[key] for i, key in enumerate(clust_order)}
        labels_covs = {
...
def _solution_plot(
    ...
    if clustered:
        if n_cov is None:
            clust_order = _get_order(
                sample_covariance, method="average", metric="euclidean"
            )

            S = hierarchical_clustering(
                sample_covariance, clust_order=clust_order, n_cov=n_cov
            )
            Theta = hierarchical_clustering(
                precision, clust_order=clust_order, n_cov=n_cov
            )
    ...
    p2 = _make_heatmap(
        ...
        labels_dict=labels_dict,
        labels_dict_reversed=labels_dict_reversed,
        title="Estimated (negative) inverse covariance",
        width=width,
        height=height,
        label_size=label_size,
    )
    ...
    try:
        ...
        if clustered:
            ...
            L = hierarchical_clustering(
                low_rank, clust_order=clust_order, n_covariates=n_cov
            )
